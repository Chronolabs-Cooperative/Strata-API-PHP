...
if (!function_exists("newakeGetStrata"))
{
    ...
    {
        $sql = "SELECT * FROM `" . $GLOBALS['APIDB']->prefix('strata_realms') . '`';
        if ($length>=2) 
            $sql .= ' WHERE `active` = "Yes" AND LENGTH(`realm`) <= '. $length;
        else 
            $sql .= ' WHERE `active` = "Yes" ';
        if (strlen($start)>=1)
            $sql .= ' HAVING `realm` LIKE "'. strtolower($start) . '%"';
        $sql .= ' ORDER BY `realm` ASC';
        $result = $GLOBALS["APIDB"]->queryF($sql);
        $results = array();
        while($row = $GLOBALS["APIDB"]->fetchArray($result))
        {
